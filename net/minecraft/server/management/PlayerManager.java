...
public class PlayerManager {
  ...
  public void updatePlayerInstances() {
    ...
    while (iterator.hasNext()) {
      ...
      if (!set1.isEmpty()) {
        ...
        int i = this.playerViewRadius / 6 + 1;
        ...
        for (ChunkCoordIntPair chunkcoordintpair : getNearest(set1, entityplayermp, i)) {
          PlayerInstance playermanager$playerinstance = getPlayerInstance(chunkcoordintpair.chunkXPos, chunkcoordintpair.chunkZPos, true);
          ...
          for (int j1 = j - radius; j1 <= j + radius; j1++) {
            for (int k1 = k - radius; k1 <= k + radius; k1++) {
              if (Config.isLazyChunkLoading()) {
                ...
                PlayerInstance playermanager$playerinstance1 = getPlayerInstance(j1, k1, true);
        ...
        for (int l = j - this.playerViewRadius; l <= j + this.playerViewRadius; l++) {
          for (int i1 = k - this.playerViewRadius; i1 <= k + this.playerViewRadius; i1++) {
            if (!overlaps(l, i1, j, k, radius)) {
              ...
              PlayerInstance playermanager$playerinstance = getPlayerInstance(l, i1, true);
              if (playermanager$playerinstance != null)
  ...
  private PriorityQueue<ChunkCoordIntPair> getNearest(Set<ChunkCoordIntPair> p_getNearest_1_, EntityPlayerMP p_getNearest_2_, int p_getNearest_3_) {
    ...
    Comparator<ChunkCoordIntPair> comparator = Collections.reverseOrder((Comparator<ChunkCoordIntPair>)chunkposcomparator);
  ...
  private Set<ChunkCoordIntPair> getPendingEntriesSafe(EntityPlayerMP p_getPendingEntriesSafe_1_) {
    ...
    int l = j * j - k * k;
    l = Math.max(l, 16);
    HashSet<ChunkCoordIntPair> hashset = new HashSet(l);
          ...
          for (int k = 0; k < 16; k++) {
            if ((this.flagsYAreasToUpdate & 1 << k) != 0) {
              ...
              for (int i1 = 0; i1 < list.size(); i1++)
                sendTileToAllPlayersWatchingChunk(list.get(i1)); 
            } 
          } 
        } 
