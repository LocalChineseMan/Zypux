...
public class Chunk {
  ...
  private void relightBlock(int x, int y, int z) {
    ...
    int j = Math.max(y, i);
    for (; j > 0 && getBlockLightOpacity(x, j - 1, z) == 0; j--);
    if (j != i) {
  ...
  public Block getBlock(int x, int y, int z) {
    try {
      ...
      crashreportcategory.addCrashSectionCallable("Location", () -> CrashReportCategory.getCoordinateInfo(new BlockPos(this.xPosition * 16 + x, y, this.zPosition * 16 + z)));
  ...
  public void onChunkLoad() {
    ...
    for (ClassInheritanceMultiMap<Entity> entityList : this.entityLists) {
      for (Entity entity : entityList)
        entity.onChunkLoad(); 
      this.worldObj.loadEntities((Collection)entityList);
    } 
  }
  ...
  public void onChunkUnload() {
    ...
    for (ClassInheritanceMultiMap<Entity> entityList : this.entityLists)
      this.worldObj.unloadEntities((Collection)entityList); 
  }
  ...
  public Random getRandomWithSeed(long seed) {
    return new Random(this.worldObj.getSeed() + this.xPosition * this.xPosition * 4987142L + this.xPosition * 5947611L + this.zPosition * this.zPosition * 4392871L + this.zPosition * 389711L ^ seed);
  }
