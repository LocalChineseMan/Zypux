...
public abstract class Entity implements ICommandSender {
  ...
  public void moveEntity(double x, double y, double z) {
    ...
    if (entity == (Minecraft.getMinecraft()).thePlayer) {
      ...
      event.call();
      safewalk = event.isSafeWalk();
      ground = event.isOnGround();
      x = event.getX();
      y = event.getY();
      z = event.getZ();
    } 
    if (this.noClip) {
      ...
      double d4 = y;
      double d5 = z;
      boolean flag = (this.onGround && (isSneaking() || safewalk) && entity instanceof EntityPlayer);
      if (flag) {
        double d6;
  ...
  protected void dealFireDamage() {
    if (!this.isImmuneToFire)
      attackEntityFrom(DamageSource.inFire, 1.0F); 
  }
  ...
  public boolean isWet() {
    return (this.inWater || this.worldObj.isRainingAt(new BlockPos(this.posX, this.posY, this.posZ)) || this.worldObj.isRainingAt(new BlockPos(this.posX, this.posY + this.height, this.posZ)));
  }
  ...
  public boolean isEntityInsideOpaqueBlock() {
    if (!this.noClip) {
      ...
      for (int i = 0; i < 8; i++) {
        int j = MathHelper.floor_double(this.posY + (((i % 2) - 0.5F) * 0.1F) + getEyeHeight());
  ...
  public String toString() {
    return String.format("%s['%s'/%d, l='%s', x=%.2f, y=%.2f, z=%.2f]", new Object[] { getClass().getSimpleName(), getName(), Integer.valueOf(this.entityId), (this.worldObj == null) ? "~NULL~" : this.worldObj.getWorldInfo().getWorldName(), Double.valueOf(this.posX), Double.valueOf(this.posY), Double.valueOf(this.posZ) });
  }
  ...
  public void addEntityCrashInfo(CrashReportCategory category) {
    ...
    category.addCrashSectionCallable("Entity's Rider", () -> this.riddenByEntity.toString());
    category.addCrashSectionCallable("Entity's Vehicle", () -> this.ridingEntity.toString());
  }
