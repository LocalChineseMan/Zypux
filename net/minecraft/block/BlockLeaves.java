...
public abstract class BlockLeaves extends BlockLeavesBase {
      ...
      for (int j1 = -i; j1 <= i; j1++) {
        for (int k1 = -i; k1 <= i; k1++) {
          for (int l1 = -i; l1 <= i; l1++) {
            ...
            if (iblockstate.getBlock().getMaterial() == Material.leaves && !((Boolean)iblockstate.getValue((IProperty)CHECK_DECAY)).booleanValue())
              worldIn.setBlockState(blockpos, iblockstate.withProperty((IProperty)CHECK_DECAY, Boolean.valueOf(true)), 4); 
          } 
        } 
      }  
  }
  ...
  public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
    if (!worldIn.isRemote)
      if (((Boolean)state.getValue((IProperty)CHECK_DECAY)).booleanValue() && ((Boolean)state.getValue((IProperty)DECAYABLE)).booleanValue()) {
        int i = 4;
        int j = i + 1;
        int k = pos.getX();
        int l = pos.getY();
        ...
        if (l2 >= 0) {
          worldIn.setBlockState(pos, state.withProperty((IProperty)CHECK_DECAY, Boolean.valueOf(false)), 4);
        } else {
          destroy(worldIn, pos);
        } 
      }  
  ...
  public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
    if (worldIn.isRainingAt(pos.up()) && !World.doesBlockHaveSolidTopSurface((IBlockAccess)worldIn, pos.down()) && rand.nextInt(15) == 1) {
