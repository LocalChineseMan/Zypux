...
import ir.lecer.uwu.events.events.Render3DEvent;
...
public class EntityRenderer implements IResourceManagerReloadListener {
  ...
  private static final ResourceLocation[] shaderResourceLocations = new ResourceLocation[] { 
      new ResourceLocation("shaders/post/notch.json"), new ResourceLocation("shaders/post/fxaa.json"), new ResourceLocation("shaders/post/art.json"), new ResourceLocation("shaders/post/bumpy.json"), new ResourceLocation("shaders/post/blobs2.json"), new ResourceLocation("shaders/post/pencil.json"), new ResourceLocation("shaders/post/color_convolve.json"), new ResourceLocation("shaders/post/deconverge.json"), new ResourceLocation("shaders/post/flip.json"), new ResourceLocation("shaders/post/invert.json"), 
      new ResourceLocation("shaders/post/ntsc.json"), new ResourceLocation("shaders/post/outline.json"), new ResourceLocation("shaders/post/phosphor.json"), new ResourceLocation("shaders/post/scan_pincushion.json"), new ResourceLocation("shaders/post/sobel.json"), new ResourceLocation("shaders/post/bits.json"), new ResourceLocation("shaders/post/desaturate.json"), new ResourceLocation("shaders/post/green.json"), new ResourceLocation("shaders/post/blur.json"), new ResourceLocation("shaders/post/wobble.json"), 
  ...
  public void loadEntityShader(Entity entityIn) {
    if (OpenGlHelper.shadersSupported) {
      if (this.theShaderGroup != null)
        this.theShaderGroup.deleteShaderGroup(); 
      this.theShaderGroup = null;
      if (entityIn instanceof net.minecraft.entity.monster.EntityCreeper) {
        loadShader(new ResourceLocation("shaders/post/creeper.json"));
      } else if (entityIn instanceof net.minecraft.entity.monster.EntitySpider) {
        loadShader(new ResourceLocation("shaders/post/spider.json"));
      } else if (entityIn instanceof net.minecraft.entity.monster.EntityEnderman) {
  ...
  public void activateNextShader() {
    if (OpenGlHelper.shadersSupported && this.mc.getRenderViewEntity() instanceof EntityPlayer) {
      ...
      if (this.shaderIndex != shaderCount) {
        loadShader(shaderResourceLocations[this.shaderIndex]);
      } else {
        this.theShaderGroup = null;
      } 
    } 
  ...
  private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
    ...
    Block block = ActiveRenderInfo.getBlockAtEntityViewpoint((World)this.mc.theWorld, entity, partialTicks);
  ...
  private void orientCamera(float partialTicks) {
    ...
    if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping()) {
      ...
      getClass();
      double d3 = (this.thirdPersonDistanceTemp + (4.0F - this.thirdPersonDistanceTemp) * partialTicks);
      if (this.mc.gameSettings.debugCamEnable) {
        ...
        for (int i = 0; i < 8; i++) {
          ...
          f4 *= 0.1F;
          f5 *= 0.1F;
          MovingObjectPosition movingobjectposition = this.mc.theWorld.rayTraceBlocks(new Vec3(d0 + f3, d1 + f4, d2 + f5), new Vec3(d0 - d4 + f3 + f5, d1 - d6 + f4, d2 - d5 + f5));
          if (movingobjectposition != null) {
    ...
    if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
      if (!this.mc.gameSettings.debugCamEnable) {
        ...
        Block block1 = ActiveRenderInfo.getBlockAtEntityViewpoint((World)this.mc.theWorld, entity, partialTicks);
  ...
  public void renderHand(float p_renderHand_1_, int p_renderHand_2_, boolean p_renderHand_3_, boolean p_renderHand_4_, boolean p_renderHand_5_) {
    ...
    Project.gluPerspective(getFOVModifier(p_renderHand_1_, false), this.mc.displayWidth / this.mc.displayHeight, 0.05F, this.farPlaneDistance * 2.0F);
    GlStateManager.matrixMode(5888);
    GlStateManager.loadIdentity();
    if (this.mc.gameSettings.anaglyph)
  ...
  private void updateLightmap(float partialTicks) {
    if (this.lightmapUpdateNeeded) {
      ...
      if (worldClient != null) {
        if (Config.isCustomColors() && CustomColors.updateLightmap((World)worldClient, this.torchFlickerX, this.lightmapColors, this.mc.thePlayer.isPotionActive(Potion.nightVision), partialTicks)) {
        ...
        for (int i = 0; i < 256; i++) {
          float f2 = ((World)worldClient).provider.getLightBrightnessTable()[i / 16] * f1;
          float f3 = ((World)worldClient).provider.getLightBrightnessTable()[i % 16] * (this.torchFlickerX * 0.1F + 1.5F);
          if (worldClient.getLastLightningBolt() > 0)
            f2 = ((World)worldClient).provider.getLightBrightnessTable()[i / 16]; 
          ...
          if (((World)worldClient).provider.getDimensionId() == 1) {
            f8 = 0.22F + f3 * 0.75F;
            f9 = 0.28F + f6 * 0.75F;
            f10 = 0.25F + f7 * 0.75F;
          } 
          if (this.mc.thePlayer.isPotionActive(Potion.nightVision)) {
            float f15 = getNightVisionBrightness((EntityLivingBase)this.mc.thePlayer, partialTicks);
            float f12 = 1.0F / f8;
            if (f12 > 1.0F / f9)
              f12 = 1.0F / f9; 
            if (f12 > 1.0F / f10)
    ...
    if (!this.mc.skipRenderWorld) {
      ...
      if (this.mc.currentScreen != null) {
        ...
        try {
          ...
          crashreportcategory.addCrashSectionCallable("Mouse location", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d)", new Object[] { Integer.valueOf(k1), Integer.valueOf(l1), Integer.valueOf(Mouse.getX()), Integer.valueOf(Mouse.getY()) }));
    ...
    if (flag && !((EntityPlayer)entity).capabilities.allowEdit) {
      ...
      if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
        ...
        if (this.mc.playerController.getCurrentGameType() == WorldSettings.GameType.SPECTATOR) {
          flag = (ReflectorForge.blockHasTileEntity(iblockstate) && this.mc.theWorld.getTileEntity(blockpos) instanceof net.minecraft.inventory.IInventory);
        } else {
  ...
  private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
    ...
    if (flag) {
      ShadersRender.setFrustrumPosition((ICamera)frustum, d0, d1, d2);
    } else {
    ...
    checkLoadVisibleChunks(entity, partialTicks, (ICamera)frustum, this.mc.thePlayer.isSpectator());
    if (flag) {
      ShadersRender.setupTerrain(renderglobal, entity, partialTicks, (ICamera)frustum, this.frameCount++, this.mc.thePlayer.isSpectator());
    } else {
      renderglobal.setupTerrain(entity, partialTicks, (ICamera)frustum, this.frameCount++, this.mc.thePlayer.isSpectator());
    } 
    if (pass == 0 || pass == 2) {
    ...
    boolean disableRainSnow = (Zypux.getInstance()).settingsManager.getSettingByName("Disable Rain/Snow").isBooleanValue();
    ...
    if (Reflector.ForgeHooksClient_setRenderPass.exists()) {
      ...
      this.mc.mcProfiler.endStartSection("entities");
      Reflector.callVoid(Reflector.ForgeHooksClient_setRenderPass, new Object[] { Integer.valueOf(1) });
      this.mc.renderGlobal.renderEntities(entity, (ICamera)frustum, partialTicks);
      GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
      Reflector.callVoid(Reflector.ForgeHooksClient_setRenderPass, new Object[] { Integer.valueOf(-1) });
      RenderHelper.disableStandardItemLighting();
    } 
  ...
  private void addRainParticles() {
    ...
    if (f != 0.0F && Config.isRainSplash()) {
      ...
      for (int l = 0; l < k; l++) {
        BlockPos blockpos1 = worldClient.getPrecipitationHeight(blockpos.add(this.random.nextInt(i) - this.random.nextInt(i), 0, this.random.nextInt(i) - this.random.nextInt(i)));
        ...
        if (blockpos1.getY() <= blockpos.getY() + i && blockpos1.getY() >= blockpos.getY() - i && biomegenbase.canRain() && biomegenbase.getFloatTemperature(blockpos1) >= 0.15F) {
          ...
          if (block.getMaterial() == Material.lava) {
            this.mc.theWorld.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, blockpos1.getX() + d3, (blockpos1.getY() + 0.1F) - block.getBlockBoundsMinY(), blockpos1.getZ() + d4, 0.0D, 0.0D, 0.0D, new int[0]);
          } else if (block.getMaterial() != Material.air) {
            block.setBlockBoundsBasedOnState((IBlockAccess)worldClient, blockpos2);
            j++;
            ...
            this.mc.theWorld.spawnParticle(EnumParticleTypes.WATER_DROP, blockpos2.getX() + d3, (blockpos2.getY() + 0.1F) + block.getBlockBoundsMaxY(), blockpos2.getZ() + d4, 0.0D, 0.0D, 0.0D, new int[0]);
          } 
        } 
      } 
      if (j > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
        this.rainSoundCounter = 0;
        if (d1 > (blockpos.getY() + 1) && worldClient.getPrecipitationHeight(blockpos).getY() > MathHelper.floor_float(blockpos.getY())) {
    ...
    if (f5 > 0.0F) {
      ...
      for (int k1 = k - i1; k1 <= k + i1; k1++) {
        for (int l1 = i - i1; l1 <= i + i1; l1++) {
          ...
          BiomeGenBase biomegenbase = worldClient.getBiomeGenForCoords((BlockPos)blockpos$mutableblockpos);
          if (biomegenbase.canRain() || biomegenbase.getEnableSnow()) {
            ...
            if (k2 != l2) {
              ...
              if (worldClient.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F) {
                if (j1 != 0) {
                  if (j1 >= 0)
                    tessellator.draw(); 
                  j1 = 0;
                ...
                double d5 = ((this.rendererUpdateCount + l1 * l1 * 3121 + l1 * 45238971 + k1 * k1 * 418711 + k1 * 13761 & 0x1F) + partialTicks) / 32.0D * (3.0D + this.random.nextDouble());
                ...
                int j3 = worldClient.getCombinedLight((BlockPos)blockpos$mutableblockpos, 0);
                int k3 = j3 >> 16 & 0xFFFF;
                int l3 = j3 & 0xFFFF;
                ...
                blockpos$mutableblockpos.set(l1, i3, k1);
                int i4 = (worldClient.getCombinedLight((BlockPos)blockpos$mutableblockpos, 0) * 3 + 15728880) / 4;
                int j4 = i4 >> 16 & 0xFFFF;
                int k4 = i4 & 0xFFFF;
  ...
  private void updateFogColor(float partialTicks) {
    ...
    vec3 = CustomColors.getWorldSkyColor(vec3, (World)worldClient, this.mc.getRenderViewEntity(), partialTicks);
    float f1 = (float)vec3.xCoord;
    float f2 = (float)vec3.yCoord;
    float f3 = (float)vec3.zCoord;
    Vec3 vec31 = worldClient.getFogColor(partialTicks);
    vec31 = CustomColors.getWorldFogColor(vec31, (World)worldClient, this.mc.getRenderViewEntity(), partialTicks);
    ...
    if (this.mc.gameSettings.renderDistanceChunks >= 4) {
      ...
      if (f5 < 0.0F)
        f5 = 0.0F; 
      if (f5 > 0.0F) {
        float[] afloat = ((World)worldClient).provider.calcSunriseSunsetColors(worldClient.getCelestialAngle(partialTicks), partialTicks);
        if (afloat != null) {
          f5 *= afloat[3];
    ...
    if (this.cloudFog) {
      ...
      Vec3 vec35 = CustomColors.getUnderwaterColor((IBlockAccess)this.mc.theWorld, (this.mc.getRenderViewEntity()).posX, (this.mc.getRenderViewEntity()).posY + 1.0D, (this.mc.getRenderViewEntity()).posZ);
      if (vec35 != null) {
      ...
      Vec3 vec34 = CustomColors.getUnderlavaColor((IBlockAccess)this.mc.theWorld, (this.mc.getRenderViewEntity()).posX, (this.mc.getRenderViewEntity()).posY + 1.0D, (this.mc.getRenderViewEntity()).posZ);
      if (vec34 != null) {
    ...
    double d1 = (entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * partialTicks) * ((World)worldClient).provider.getVoidFogYFactor();
  ...
  private void setupFog(int startCoords, float partialTicks) {
    ...
    Entity entity = this.mc.getRenderViewEntity();
    GL11.glFog(2918, setFogColorBuffer(this.fogColorRed, this.fogColorGreen, this.fogColorBlue));
    GL11.glNormal3f(0.0F, -1.0F, 0.0F);
    GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
    Block block = ActiveRenderInfo.getBlockAtEntityViewpoint((World)this.mc.theWorld, entity, partialTicks);
    float f = -1.0F;
  ...
  private FloatBuffer setFogColorBuffer(float red, float green, float blue) {
    ...
    this.fogColorBuffer.put(red).put(green).put(blue).put(1.0F);
    ...
    if (Config.isSmoothWorld() && Config.isSingleProcessor()) {
      if (this.mc.isIntegratedServerRunning()) {
        ...
        if (integratedserver != null) {
          ...
          if (!flag && !(this.mc.currentScreen instanceof net.minecraft.client.gui.GuiDownloadTerrain)) {
  ...
  private void frameInit() {
    ...
    if (this.updatedWorld != worldClient) {
      RandomEntities.worldChanged(this.updatedWorld, (World)worldClient);
      Config.updateThreadPriorities();
      this.lastServerTime = 0L;
      this.lastServerTicks = 0;
      this.updatedWorld = (World)worldClient;
    } 
  ...
  private void updateMainMenu(GuiMainMenu p_updateMainMenu_1_) {
    ...
    Reflector.setFieldValue(p_updateMainMenu_1_, Reflector.GuiMainMenu_splashText, null);
  }
  ...
  private void checkLoadVisibleChunks(Entity p_checkLoadVisibleChunks_1_, float p_checkLoadVisibleChunks_2_, ICamera p_checkLoadVisibleChunks_3_, boolean p_checkLoadVisibleChunks_4_) {
    ...
    if (Keyboard.isKeyDown(61) && Keyboard.isKeyDown(38)) {
      ...
      this.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion((IChatComponent)chatcomponenttext, i);
    } 
  }
