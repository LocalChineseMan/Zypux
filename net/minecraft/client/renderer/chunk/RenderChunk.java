...
public class RenderChunk {
  ...
  public void resortTransparency(float x, float y, float z, ChunkCompileTaskGenerator generator) {
    ...
    if (compiledchunk.getState() != null && !compiledchunk.isLayerEmpty(EnumWorldBlockLayer.TRANSLUCENT)) {
      ...
      preRenderBlocks(worldrenderer, this.position);
  ...
  public void rebuildChunk(float x, float y, float z, ChunkCompileTaskGenerator generator) {
    ...
    if (!isChunkRegionEmpty(blockpos)) {
      ...
      for (Object blockposm0 : BlockPosM.getAllInBoxMutable(blockpos, blockpos1)) {
        ...
        IBlockState iblockstate = chunkcacheof.getBlockState((BlockPos)blockposm);
        Block block = iblockstate.getBlock();
        if (block.isOpaqueCube())
          lvt_10_1_.func_178606_a((BlockPos)blockposm); 
        if (ReflectorForge.blockHasTileEntity(iblockstate)) {
          TileEntity tileentity = chunkcacheof.getTileEntity(new BlockPos((Vec3i)blockposm));
        ...
        for (int j = 0; j < aenumworldblocklayer.length; j++) {
          ...
          if (block.getRenderType() != -1) {
            ...
            RenderEnv renderenv = worldrenderer.getRenderEnv(iblockstate, (BlockPos)blockposm);
            ...
            aboolean[k] = aboolean[k] | blockrendererdispatcher.renderBlock(iblockstate, (BlockPos)blockposm, (IBlockAccess)chunkcacheof, worldrenderer);
  ...
  private EnumWorldBlockLayer fixBlockLayer(IBlockState p_fixBlockLayer_1_, EnumWorldBlockLayer p_fixBlockLayer_2_) {
    ...
    if (this.isMipmaps) {
      if (p_fixBlockLayer_2_ == EnumWorldBlockLayer.CUTOUT) {
        ...
        if (block instanceof net.minecraft.block.BlockRedstoneWire)
          return p_fixBlockLayer_2_; 
  ...
  private ChunkCacheOF makeChunkCacheOF(BlockPos p_makeChunkCacheOF_1_) {
    ...
    RegionRenderCache regionRenderCache = createRegionRenderCache(this.world, blockpos, blockpos1, 1);
    if (Reflector.MinecraftForgeClient_onRebuildChunk.exists())
      Reflector.call(Reflector.MinecraftForgeClient_onRebuildChunk, new Object[] { this.world, p_makeChunkCacheOF_1_, regionRenderCache }); 
    ChunkCacheOF chunkcacheof = new ChunkCacheOF((ChunkCache)regionRenderCache, blockpos, blockpos1, 1);
    return chunkcacheof;
  }
  ...
  public boolean isBoundingBoxInFrustum(ICamera p_isBoundingBoxInFrustum_1_, int p_isBoundingBoxInFrustum_2_) {
    return getBoundingBoxParent().isBoundingBoxInFrustumFully(p_isBoundingBoxInFrustum_1_, p_isBoundingBoxInFrustum_2_) ? true : p_isBoundingBoxInFrustum_1_.isBoundingBoxInFrustum(this.boundingBox);
  }
