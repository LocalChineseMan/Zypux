...
public class WorldRenderer {
  ...
  public void sortVertexData(float p_181674_1_, float p_181674_2_, float p_181674_3_) {
    ...
    for (int l1 = 0; (l1 = bitset.nextClearBit(l1)) < ainteger.length; l1++) {
      ...
      if (i1 != l1) {
        ...
        int j1 = i1;
        int k1;
        for (k1 = ainteger[i1].intValue(); j1 != l1; k1 = ainteger[k1].intValue()) {
  ...
  public void putColorMultiplierRgba(float p_putColorMultiplierRgba_1_, float p_putColorMultiplierRgba_2_, float p_putColorMultiplierRgba_3_, float p_putColorMultiplierRgba_4_, int p_putColorMultiplierRgba_5_) {
    ...
    if (!this.noColor) {
      j = this.rawIntBuffer.get(i);
      if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) {
        int k = (int)((j & 0xFF) * p_putColorMultiplierRgba_1_);
        int l = (int)((j >> 8 & 0xFF) * p_putColorMultiplierRgba_2_);
        int i1 = (int)((j >> 16 & 0xFF) * p_putColorMultiplierRgba_3_);
        int j1 = (int)((j >> 24 & 0xFF) * p_putColorMultiplierRgba_4_);
        j = j1 << 24 | i1 << 16 | l << 8 | k;
      } else {
        int k1 = (int)((j >> 24 & 0xFF) * p_putColorMultiplierRgba_1_);
        int l1 = (int)((j >> 16 & 0xFF) * p_putColorMultiplierRgba_2_);
        int i2 = (int)((j >> 8 & 0xFF) * p_putColorMultiplierRgba_3_);
        int j2 = (int)((j & 0xFF) * p_putColorMultiplierRgba_4_);
