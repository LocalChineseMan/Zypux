...
public abstract class GuiContainer extends GuiScreen {
  ...
  public void initGui() {
    GuiContainerInitEvent guiContainerInitEvent = new GuiContainerInitEvent();
    guiContainerInitEvent.call();
    super.initGui();
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    for (Module module : Category.HUD.getModules()) {
      if (module.getName().equalsIgnoreCase("ui designer")) {
        boolean customBG = (Zypux.getInstance()).settingsManager.getSettingByName("Custom Background").isBooleanValue();
        if (!customBG) {
          drawDefaultBackground();
        } else {
          drawGradientRect(0, 0, width, height, (new Color(0, true))
              .getRGB(), (new Color(0, true))
              .getRGB());
          RenderUtils.drawGradientRect(0.0D, 0.0D, width, height, (new Color(1291845632, true))
              .getRGB(), 
              ColorUtils.reAlpha(ColorUtils.rainbow(10), 0.3F));
        } 
      } 
    } 
    GuiContainerRenderEvent guiContainerRenderEvent = new GuiContainerRenderEvent();
    guiContainerRenderEvent.call();
    int i = this.guiLeft;
    int j = this.guiTop;
