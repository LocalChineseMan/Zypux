...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
  ...
  public NetHandlerPlayClient(Minecraft mcIn, GuiScreen p_i46300_2_, NetworkManager p_i46300_3_, GameProfile p_i46300_4_) {
    ...
    this.networkManager = p_i46300_3_;
  ...
  public void handleJoinGame(S01PacketJoinGame packetIn) {
    ...
    this.networkManager.sendPacket((Packet)new C17PacketCustomPayload("MC|Brand", (new PacketBuffer(Unpooled.buffer())).writeString(ClientBrandRetriever.getClientModName())));
  }
  ...
  public void handleSpawnObject(S0EPacketSpawnObject packetIn) {
    ...
    if (packetIn.getType() == 10) {
      ...
      EntityFireworkRocket entityFireworkRocket = new EntityFireworkRocket((World)this.clientWorldController, d0, d1, d2, null);
  ...
  public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
    ...
    this.networkManager.sendPacket((Packet)new C03PacketPlayer.C06PacketPlayerPosLook(((EntityPlayer)entityPlayerSP).posX, (entityPlayerSP.getEntityBoundingBox()).minY, ((EntityPlayer)entityPlayerSP).posZ, ((EntityPlayer)entityPlayerSP).rotationYaw, ((EntityPlayer)entityPlayerSP).rotationPitch, false));
  ...
  public void handleExplosion(S27PacketExplosion packetIn) {
    PacketThreadUtil.checkThreadAndEnqueue((Packet)packetIn, (INetHandler)this, (IThreadListener)this.gameController);
    Explosion explosion = new Explosion((World)this.gameController.theWorld, null, packetIn.getX(), packetIn.getY(), packetIn.getZ(), packetIn.getStrength(), packetIn.getAffectedBlockPositions());
  ...
  public void handleCombatEvent(S42PacketCombatEvent packetIn) {
    ...
    if (packetIn.eventType == S42PacketCombatEvent.Event.END_COMBAT) {
      long i = 1000L * packetIn.field_179772_d / 20L;
      MetadataCombat metadatacombat = new MetadataCombat((EntityLivingBase)this.gameController.thePlayer, entitylivingbase);
      this.gameController.getTwitchStream().func_176026_a((Metadata)metadatacombat, -i, 0L);
  ...
  public void handleResourcePack(S48PacketResourcePackSend packetIn) {
    ...
    if (s.startsWith("level://")) {
      ...
      if (file2.isFile()) {
        this.networkManager.sendPacket((Packet)new C19PacketResourcePackStatus(s1, C19PacketResourcePackStatus.Action.ACCEPTED));
        Futures.addCallback(this.gameController.getResourcePackRepository().setResourcePackInstance(file2), new FutureCallback<Object>() {
              public void onSuccess(Object p_onSuccess_1_) {
                NetHandlerPlayClient.this.networkManager.sendPacket((Packet)new C19PacketResourcePackStatus(s1, C19PacketResourcePackStatus.Action.SUCCESSFULLY_LOADED));
              }
              
              public void onFailure(@NotNull Throwable p_onFailure_1_) {
                NetHandlerPlayClient.this.networkManager.sendPacket((Packet)new C19PacketResourcePackStatus(s1, C19PacketResourcePackStatus.Action.FAILED_DOWNLOAD));
              }
            });
      } else {
        this.networkManager.sendPacket((Packet)new C19PacketResourcePackStatus(s1, C19PacketResourcePackStatus.Action.FAILED_DOWNLOAD));
      } 
      ...
      this.networkManager.sendPacket((Packet)new C19PacketResourcePackStatus(s1, C19PacketResourcePackStatus.Action.DECLINED));
    } else {
      this.gameController.addScheduledTask(() -> this.gameController.displayGuiScreen((GuiScreen)new GuiYesNo((), I18n.format("multiplayer.texturePrompt.line1", new Object[0]), I18n.format("multiplayer.texturePrompt.line2", new Object[0]), 0)));
    } 
  }
    ...
    if (entity != null) {
      ...
      for (S20PacketEntityProperties.Snapshot s20packetentityproperties$snapshot : packetIn.func_149441_d()) {
        ...
        if (iattributeinstance == null)
          iattributeinstance = baseattributemap.registerAttribute((IAttribute)new RangedAttribute(null, s20packetentityproperties$snapshot.func_151409_a(), 0.0D, 2.2250738585072014E-308D, Double.MAX_VALUE)); 
