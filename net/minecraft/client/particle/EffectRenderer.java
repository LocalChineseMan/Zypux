...
public class EffectRenderer {
  ...
  private List<EntityFX>[][] fxLayers = (List<EntityFX>[][])new List[4][];
  ...
  private void updateEffectLayer(int layer) {
    for (int i = 0; i < 2; i++)
      updateEffectAlphaLayer(this.fxLayers[layer][i]); 
  }
      ...
      for (int i = 0; i < l; i++) {
        for (int j = 0; j < l; j++) {
          for (int k = 0; k < l; k++) {
            ...
            addEffect((new EntityDiggingFX(this.worldObj, d0, d1, d2, d0 - pos.getX() - 0.5D, d1 - pos.getY() - 0.5D, d2 - pos.getZ() - 0.5D, state)).setBlockPos(pos));
          } 
        } 
      } 
    } 
  ...
  public void addBlockHitEffects(BlockPos pos, EnumFacing side) {
    ...
    if (block.getRenderType() != -1) {
      ...
      addEffect((new EntityDiggingFX(this.worldObj, d0, d1, d2, 0.0D, 0.0D, 0.0D, iblockstate)).setBlockPos(pos).multiplyVelocity(0.2F).multipleParticleScaleBy(0.6F));
    } 
  }
