  ...
  public DirectInputEnvironmentPlugin() {
    ...
    if (isSupported()) {
      ...
      AccessController.doPrivileged(new PrivilegedAction() {
            ...
            public final Object run() {
              Runtime.getRuntime().addShutdownHook(new DirectInputEnvironmentPlugin.ShutdownHook());
              return null;
            }
          });
    } else {
  ...
  private final class ShutdownHook extends Thread {
    ...
    public final void run() {
      for (int i = 0; i < DirectInputEnvironmentPlugin.this.active_devices.size(); i++) {
        IDirectInputDevice device = DirectInputEnvironmentPlugin.this.active_devices.get(i);
        device.release();
      } 
    }
  }
