...
public final class LinuxEnvironmentPlugin extends ControllerEnvironment implements Plugin {
  ...
  public LinuxEnvironmentPlugin() {
    if (isSupported()) {
      ...
      AccessController.doPrivileged(new PrivilegedAction() {
            ...
            public final Object run() {
              Runtime.getRuntime().addShutdownHook(new LinuxEnvironmentPlugin.ShutdownHook());
              return null;
            }
          });
    } else {
  ...
  private static final Controller createJoystickFromJoystickDevice(LinuxJoystickDevice device) {
    ...
    return new LinuxJoystickAbstractController(device, components.<Component>toArray(new Component[0]), new Controller[0], new Rumbler[0]);
  }
  ...
  private static File[] listFilesPrivileged(final File dir, final FilenameFilter filter) {
    return AccessController.<File[]>doPrivileged(new PrivilegedAction() {
          ...
          public Object run() {
            ...
            Arrays.sort(files, new Comparator() {
                  private final LinuxEnvironmentPlugin.null this$0;
  ...
  private final class ShutdownHook extends Thread {
    ...
    public final void run() {
      for (int i = 0; i < LinuxEnvironmentPlugin.this.devices.size(); i++) {
        try {
          LinuxDevice device = LinuxEnvironmentPlugin.this.devices.get(i);
          device.close();
        } catch (IOException e) {
