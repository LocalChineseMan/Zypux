...
public class DynamicLights {
  ...
  private static Map<Class, Integer> mapEntityLightLevels = (Map)new HashMap<>();
  ...
  private static void loadModConfiguration(InputStream in, String path, String modId) {
    if (in != null)
      try {
        ...
        loadModLightLevels(propertiesOrdered.getProperty("entities"), mapEntityLightLevels, (IObjectLocator)new EntityClassLocator(), connectedparser, path, modId);
        loadModLightLevels(propertiesOrdered.getProperty("items"), mapItemLightLevels, (IObjectLocator)new ItemLocator(), connectedparser, path, modId);
  ...
  private static void updateMapDynamicLights(RenderGlobal renderGlobal) {
    WorldClient worldClient = renderGlobal.getWorld();
    if (worldClient != null)
