...
public class ConnectedParser {
  ...
  public MatchBlock[] parseMatchBlocks(String propMatchBlocks) {
    ...
    for (int i = 0; i < astring.length; i++) {
      String s = astring[i];
      MatchBlock[] amatchblock = parseMatchBlock(s);
      if (amatchblock != null)
  ...
  public IBlockState parseBlockState(String str, IBlockState def) {
    MatchBlock[] amatchblock = parseMatchBlock(str);
    if (amatchblock == null)
      return def; 
  ...
  public MatchBlock[] parseMatchBlock(String blockStr) {
    ...
    String s = "minecraft";
    int i = 0;
    if (astring.length > 1 && isFullBlockName(astring)) {
      s = astring[0];
      i = 1;
    } else {
      s = "minecraft";
    ...
    String s1 = astring[i];
    String[] astring1 = Arrays.<String>copyOfRange(astring, i + 1, astring.length);
    Block[] ablock = parseBlockPart(s, s1);
    if (ablock == null)
      return null; 
    ...
    for (int j = 0; j < ablock.length; j++) {
      ...
      if (astring1.length > 0) {
        aint = parseBlockMetadatas(block, astring1);
        if (aint == null)
          return null; 
      } 
  ...
  public boolean isFullBlockName(String[] parts) {
    if (parts.length < 2)
      return false; 
    String s = parts[1];
    return (s.length() < 1) ? false : (startsWithDigit(s) ? false : (!s.contains("=")));
  }
  ...
  public Block[] parseBlockPart(String domain, String blockPart) {
    if (startsWithDigit(blockPart)) {
      int[] aint = parseIntList(blockPart);
      if (aint == null)
        return null; 
    ...
    if (block == null) {
      warn("Block not found for name: " + s);
      return null;
    } 
    Block[] ablock = { block };
    return ablock;
  ...
  public int[] parseBlockMetadatas(Block block, String[] params) {
    ...
    for (int i = 0; i < params.length; i++) {
      ...
      if (s1.length() > 0) {
        ...
        if (astring.length != 2) {
          warn("Invalid block property: " + s1);
          return null;
        } 
        String s2 = astring[0];
        String s3 = astring[1];
        ...
        if (iproperty == null) {
          warn("Property not found: " + s2 + ", block: " + block);
          return null;
        } 
        ...
        for (int j = 0; j < astring1.length; j++) {
          ...
          if (comparable == null) {
            warn("Property value not found: " + s4 + ", property: " + s2 + ", block: " + block);
            return null;
          } 
          list.add(comparable);
        } 
    ...
    for (int k = 0; k < 16; k++) {
      int l = k;
      try {
        IBlockState iblockstate1 = getStateFromMeta(block, l);
        if (matchState(iblockstate1, map))
  ...
  private IBlockState getStateFromMeta(Block block, int md) {
    try {
      ...
      if (block == Blocks.double_plant && md > 7) {
        IBlockState iblockstate1 = block.getStateFromMeta(md & 0x7);
  ...
  public BiomeGenBase[] parseBiomes(String str) {
    ...
    BiomeGenBase[] abiomegenbase = list.<BiomeGenBase>toArray(new BiomeGenBase[list.size()]);
    ...
    for (int i = 0; i < astring.length; i++) {
      ...
      if (s.contains("-")) {
        ...
        if (astring1.length != 2) {
          warn("Invalid interval: " + s + ", when parsing: " + str);
        } else {
        ...
        if (j < 0) {
          warn("Invalid number: " + s + ", when parsing: " + str);
        } else {
  ...
  public boolean[] parseFaces(String str, boolean[] defVal) {
    ...
    for (int i = 0; i < astring.length; i++) {
      ...
      if (s.equals("sides")) {
        ...
        EnumFacing enumfacing = parseFace(s);
        if (enumfacing != null)
  ...
  public <T> T[] parseObjects(String str, T[] objs, INameGetter nameGetter, String property, T[] errValue) {
    ...
    for (int i = 0; i < astring.length; i++) {
      String s = astring[i];
      T t = parseObject(s, objs, nameGetter, property);
      if (t == null)
        return errValue; 
      at[i] = t;
    } 
  ...
  public int[] parseItems(String str) {
    ...
    for (int i = 0; i < astring.length; i++) {
      ...
      if (item == null) {
        warn("Item not found: " + s);
      } else {
        int j = Item.getIdFromItem(item);
        if (j < 0) {
          warn("Item has no ID: " + item + ", name: " + s);
        } else {
