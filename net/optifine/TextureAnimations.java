  ...
  private static TextureAnimation[] getTextureAnimations(IResourcePack rp) {
    ...
    for (int i = 0; i < astring.length; i++) {
      ...
      try {
        ...
        inputstream.close();
        TextureAnimation textureanimation = makeTextureAnimation((Properties)propertiesOrdered, resourcelocation);
    ...
    TextureAnimation[] atextureanimation = list.<TextureAnimation>toArray(new TextureAnimation[list.size()]);
  ...
  private static TextureAnimation makeTextureAnimation(Properties props, ResourceLocation propLoc) {
    ...
    if (s != null && s1 != null) {
      if (i >= 0 && j >= 0 && k >= 0 && l >= 0) {
        ...
        int k1 = j1 * k * l;
        if (i1 != k1) {
          Config.warn("TextureAnimation: Source texture has invalid number of frames: " + s + ", frames: " + (i1 / (k * l)));
          return null;
        } 
