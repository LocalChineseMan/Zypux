...
public class Lagometer {
  ...
  public static void updateLagometer() {
    ...
    if (gameSettings.showDebugInfo && (gameSettings.ofLagometer || gameSettings.showLagometerR)) {
      active = true;
  ...
  public static void showLagometer(ScaledResolution scaledResolution) {
    if (gameSettings != null && (
      gameSettings.ofLagometer || gameSettings.showLagometerR)) {
      ...
      for (int j = 0; j < timesFrame.length; j++) {
        ...
        if (gcs[j]) {
          ...
          f -= (float)renderTime(j, timesServer[j], k / 2, k / 2, k / 2, f, worldrenderer);
          f -= (float)renderTime(j, timesTerrain[j], 0, k, 0, f, worldrenderer);
          f -= (float)renderTime(j, timesVisibility[j], k, k, 0, f, worldrenderer);
          f -= (float)renderTime(j, timesChunkUpdate[j], k, 0, 0, f, worldrenderer);
          f -= (float)renderTime(j, timesChunkUpload[j], k, 0, k, f, worldrenderer);
          f -= (float)renderTime(j, timesScheduledExecutables[j], 0, 0, k, f, worldrenderer);
          float f2 = f - (float)renderTime(j, timesTick[j], 0, k, k, f, worldrenderer);
        } 
      } 
      renderTimeDivider(timesFrame.length, 33333333L, 196, 196, 196, mc.displayHeight, worldrenderer);
      renderTimeDivider(timesFrame.length, 16666666L, 196, 196, 196, mc.displayHeight, worldrenderer);
      tessellator.draw();
  ...
  private static long renderTime(int frameNum, long time, int r, int g, int b, float baseHeight, WorldRenderer tessellator) {
    ...
    if (i < 3L)
      return 0L; 
    tessellator.pos((frameNum + 0.5F), (baseHeight - (float)i + 0.5F), 0.0D).color(r, g, b, 255).endVertex();
    tessellator.pos((frameNum + 0.5F), (baseHeight + 0.5F), 0.0D).color(r, g, b, 255).endVertex();
    return i;
  }
  
  private static long renderTimeDivider(int frameEnd, long time, int r, int g, int b, float baseHeight, WorldRenderer tessellator) {
    long i = time / 200000L;
    if (i < 3L)
      return 0L; 
    tessellator.pos(0.5D, (baseHeight - (float)i + 0.5F), 0.0D).color(r, g, b, 255).endVertex();
    tessellator.pos((frameEnd + 0.5F), (baseHeight - (float)i + 0.5F), 0.0D).color(r, g, b, 255).endVertex();
    return i;
  }
