...
public class ConnectedTextures {
  ...
  private static synchronized BakedQuad getQuad(TextureAtlasSprite sprite, BakedQuad quadIn) {
    ...
    if (i >= 0 && i < spriteQuadMaps.length) {
      Map<Object, Object> map = spriteQuadMaps[i];
      if (map == null) {
  ...
  private static synchronized BakedQuad getQuadFull(TextureAtlasSprite sprite, BakedQuad quadIn, int tintIndex) {
    ...
    if (i >= 0 && i < spriteQuadFullMaps.length) {
      Map<EnumFacing, Object> map = spriteQuadFullMaps[i];
      if (map == null) {
  ...
  private static BakedQuad[] getConnectedTexture(ConnectedProperties cp, IBlockAccess blockAccess, BlockStateBase blockState, BlockPos blockPos, int side, BakedQuad quad, int pass, RenderEnv renderEnv) {
    ...
    int k = j;
    Block block = blockState.getBlock();
    if (block instanceof net.minecraft.block.BlockRotatedPillar) {
      i = getWoodAxis(side, j);
      if (cp.getMetadataMax() <= 3)
        k = j & 0x3; 
    } 
    if (block instanceof net.minecraft.block.BlockQuartz) {
    ...
    switch (cp.method) {
      case 1:
        return getQuads(getConnectedTextureCtm(cp, blockAccess, (IBlockState)blockState, blockPos, i, side, textureatlassprite, j, renderEnv), quad, renderEnv);
      case 2:
        return getQuads(getConnectedTextureHorizontal(cp, blockAccess, (IBlockState)blockState, blockPos, i, side, textureatlassprite, j), quad, renderEnv);
      case 3:
        return getQuads(getConnectedTextureTop(cp, blockAccess, (IBlockState)blockState, blockPos, i, side, textureatlassprite, j), quad, renderEnv);
      case 4:
      ...
      case 6:
        return getQuads(getConnectedTextureVertical(cp, blockAccess, (IBlockState)blockState, blockPos, i, side, textureatlassprite, j), quad, renderEnv);
      case 7:
        return getQuads(getConnectedTextureFixed(cp), quad, renderEnv);
      case 8:
        return getQuads(getConnectedTextureHorizontalVertical(cp, blockAccess, (IBlockState)blockState, blockPos, i, side, textureatlassprite, j), quad, renderEnv);
      case 9:
        return getQuads(getConnectedTextureVerticalHorizontal(cp, blockAccess, (IBlockState)blockState, blockPos, i, side, textureatlassprite, j), quad, renderEnv);
      case 10:
        if (pass == 0)
          return getConnectedTextureCtmCompact(cp, blockAccess, (IBlockState)blockState, blockPos, i, side, quad, j, renderEnv); 
      default:
        return null;
      case 11:
        return getConnectedTextureOverlay(cp, blockAccess, (IBlockState)blockState, blockPos, i, side, quad, j, renderEnv);
      case 12:
    ...
    return getConnectedTextureOverlayCtm(cp, blockAccess, (IBlockState)blockState, blockPos, i, side, quad, j, renderEnv);
  }
  ...
  private static int getConnectedTextureCtmIndex(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, int vertAxis, int side, TextureAtlasSprite icon, int metadata, RenderEnv renderEnv) {
    ...
    int i = 0;
    if ((aboolean[0] & (!aboolean[1] ? 1 : 0) & (!aboolean[2] ? 1 : 0) & (!aboolean[3] ? 1 : 0)) != 0) {
      i = 3;
    } else if (((!aboolean[0] ? 1 : 0) & aboolean[1] & (!aboolean[2] ? 1 : 0) & (!aboolean[3] ? 1 : 0)) != 0) {
      i = 1;
    } else if (((!aboolean[0] ? 1 : 0) & (!aboolean[1] ? 1 : 0) & aboolean[2] & (!aboolean[3] ? 1 : 0)) != 0) {
      i = 12;
    } else if (((!aboolean[0] ? 1 : 0) & (!aboolean[1] ? 1 : 0) & (!aboolean[2] ? 1 : 0) & aboolean[3]) != 0) {
      i = 36;
    } else if ((aboolean[0] & aboolean[1] & (!aboolean[2] ? 1 : 0) & (!aboolean[3] ? 1 : 0)) != 0) {
      i = 2;
    } else if (((!aboolean[0] ? 1 : 0) & (!aboolean[1] ? 1 : 0) & aboolean[2] & aboolean[3]) != 0) {
      i = 24;
    } else if ((aboolean[0] & (!aboolean[1] ? 1 : 0) & aboolean[2] & (!aboolean[3] ? 1 : 0)) != 0) {
      i = 15;
    } else if ((aboolean[0] & (!aboolean[1] ? 1 : 0) & (!aboolean[2] ? 1 : 0) & aboolean[3]) != 0) {
      i = 39;
    } else if (((!aboolean[0] ? 1 : 0) & aboolean[1] & aboolean[2] & (!aboolean[3] ? 1 : 0)) != 0) {
      i = 13;
    } else if (((!aboolean[0] ? 1 : 0) & aboolean[1] & (!aboolean[2] ? 1 : 0) & aboolean[3]) != 0) {
      i = 37;
    } else if (((!aboolean[0] ? 1 : 0) & aboolean[1] & aboolean[2] & aboolean[3]) != 0) {
      i = 25;
    } else if ((aboolean[0] & (!aboolean[1] ? 1 : 0) & aboolean[2] & aboolean[3]) != 0) {
      i = 27;
    } else if ((aboolean[0] & aboolean[1] & (!aboolean[2] ? 1 : 0) & aboolean[3]) != 0) {
      i = 38;
    } else if ((aboolean[0] & aboolean[1] & aboolean[2] & (!aboolean[3] ? 1 : 0)) != 0) {
      i = 14;
    } else if ((aboolean[0] & aboolean[1] & aboolean[2] & aboolean[3]) != 0) {
      i = 26;
    } 
    if (i == 0)
      return i; 
  ...
  private static boolean isNeighbourOverlay(ConnectedProperties cp, IBlockAccess iblockaccess, IBlockState blockState, BlockPos blockPos, int side, TextureAtlasSprite icon, int metadata) {
    ...
    if (cp.connectTileIcons != null) {
      ...
      if (!Config.isSameOne(textureatlassprite, (Object[])cp.connectTileIcons))
        return false; 
    } 
  ...
  private static boolean isFullCubeModel(IBlockState state) {
    ...
    return (block instanceof net.minecraft.block.BlockGlass) ? true : (block instanceof net.minecraft.block.BlockStainedGlass);
  }
  ...
  public static void updateIcons(TextureMap textureMap) {
    ...
    if (Config.isConnectedTextures()) {
      ...
      updateIcons(textureMap, (IResourcePack)Config.getDefaultResourcePack());
    ...
    for (int i = 0; i < astring.length; i++) {
      ...
      try {
        ...
        if (inputstream == null) {
          ...
          inputstream.close();
          ConnectedProperties connectedproperties = new ConnectedProperties((Properties)propertiesOrdered, s);
  ...
  private static List makePropertyList(ConnectedProperties[][] propsArr) {
    ...
    if (propsArr != null)
      for (int i = 0; i < propsArr.length; i++) {
        ...
        List list1 = null;
        if (aconnectedproperties != null)
          list1 = new ArrayList(Arrays.asList((Object[])aconnectedproperties)); 
        list.add(list1);
      }  
    return list;
  }
  ...
  private static ConnectedProperties[][] propertyListToArray(List<List> list) {
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      if (list2 != null) {
        ConnectedProperties[] aconnectedproperties1 = (ConnectedProperties[])list2.toArray((Object[])new ConnectedProperties[list2.size()]);
  ...
  private static void addToList(ConnectedProperties cp, List<List> list, int id) {
    while (id >= list.size())
      list.add(null); 
    List<ConnectedProperties> list1 = list.get(id);
    if (list1 == null) {
      list1 = new ArrayList();
      list.set(id, list1);
    } 
  ...
  private static String[] getDefaultCtmPaths() {
    ...
    String[] astring = { 
        "white", "orange", "magenta", "light_blue", "yellow", "lime", "pink", "gray", "silver", "cyan", 
