...
public class MappedObjectTransformer {
  ...
  static byte[] transformMappedObject(byte[] bytecode) {
    ...
    ClassAdapter classAdapter = new ClassAdapter((ClassVisitor)cw) {
  ...
  static byte[] transformMappedAPI(String className, byte[] bytecode) {
    ...
    TransformationAdapter ta = new TransformationAdapter((ClassVisitor)cw, className);
    TransformationAdapter transformationAdapter1 = ta;
    if (className_to_subtype.containsKey(className))
      classAdapter = getMethodGenAdapter(className, (ClassVisitor)transformationAdapter1); 
  ...
  private static void printBytecode(byte[] bytecode) {
    ...
    TraceClassVisitor traceClassVisitor = new TraceClassVisitor((ClassVisitor)new ClassWriter(0), new PrintWriter(sw));
    (new ClassReader(bytecode)).accept((ClassVisitor)traceClassVisitor, 0);
    String dump = sw.toString();
    LWJGLUtil.log(dump);
  }
  
  private static class MappedObjectTransformer {}
}
