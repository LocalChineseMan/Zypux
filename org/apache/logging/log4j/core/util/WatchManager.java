...
public class WatchManager extends AbstractLifeCycle {
        ...
        if (monitor.getWatcher().isModified()) {
          ...
          if (WatchManager.logger.isInfoEnabled())
            WatchManager.logger.info("Source '{}' was modified on {} ({}), previous modification was on {} ({})", source, WatchManager.this
                .millisToString(lastModified), Long.valueOf(lastModified), WatchManager.this.millisToString(WatchManager.ConfigurationMonitor.access$300(monitor)), 
                Long.valueOf(WatchManager.ConfigurationMonitor.access$300(monitor))); 
          WatchManager.ConfigurationMonitor.access$302(monitor, lastModified);
  ...
  public WatchManager(ConfigurationScheduler scheduler) {
    ...
    this
      .eventServiceList = (List<WatchEventService>)ServiceLoaderUtil.loadServices(WatchEventService.class, MethodHandles.lookup(), true).collect(Collectors.toList());
  }
