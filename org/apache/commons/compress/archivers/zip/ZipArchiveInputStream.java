...
public class ZipArchiveInputStream extends ArchiveInputStream {
  ...
  public ZipArchiveEntry getNextZipEntry() throws IOException {
    ...
    if (this.current.entry.getCompressedSize() != -1L)
      if (this.current.entry.getMethod() == ZipMethod.UNSHRINKING.getCode()) {
        this.current.in = (InputStream)new UnshrinkingInputStream(new BoundedInputStream(this.in, this.current.entry.getCompressedSize()));
  ...
  public long skip(long value) throws IOException {
    if (value >= 0L) {
      ...
      while (skipped < value) {
        ...
        int x = read(this.SKIP_BUF, 0, (int)((this.SKIP_BUF.length > rem) ? rem : this.SKIP_BUF.length));
        if (x == -1)
          return skipped; 
        skipped += x;
      } 
  ...
  private void realSkip(long value) throws IOException {
    if (value >= 0L) {
      ...
      while (skipped < value) {
        long rem = value - skipped;
        int x = this.in.read(this.SKIP_BUF, 0, (int)((this.SKIP_BUF.length > rem) ? rem : this.SKIP_BUF.length));
        if (x == -1)
          return; 
        count(x);
        skipped += x;
