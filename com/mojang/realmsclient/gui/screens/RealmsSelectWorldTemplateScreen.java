  ...
  public void keyPressed(char eventCharacter, int eventKey) {
    switch (eventKey) {
      ...
      case 200:
        if (this.selectedTemplate != -1) {
          ...
          if (new_index > -1) {
            ...
            int maxScroll = Math.max(0, this.worldTemplateSelectionList.getMaxPosition() - height() - 40 - (this.displayWarning ? RealmsConstants.row(1) : 32) - 4);
            int maxItemsInView = (int)Math.floor(((height() - 40 - (this.displayWarning ? RealmsConstants.row(1) : 32)) / 46));
            int scroll = this.worldTemplateSelectionList.getScroll();
            int hiddenItems = (int)Math.ceil((scroll / 46.0F));
      ...
      case 208:
        if (this.selectedTemplate != -1) {
          int the_index = this.selectedTemplate;
          int maxScroll = Math.max(0, this.worldTemplateSelectionList.getMaxPosition() - height() - 40 - (this.displayWarning ? RealmsConstants.row(1) : 32));
  ...
  public void render(int xm, int ym, float a) {
    ...
    if (this.displayWarning) {
      ...
      for (index = 0; index < lines.length; index++) {
        int fontWidth = fontWidth(lines[index]);
        int offsetX = width() / 2 - fontWidth / 2;
        int offsetY = RealmsConstants.row(-1 + index);
        if (xm >= offsetX && xm <= offsetX + fontWidth && ym >= offsetY && ym <= offsetY + fontLineHeight())
      ...
      for (index = 0; index < lines.length; index++) {
        ...
        drawCenteredString(line, width() / 2, RealmsConstants.row(-1 + index), warningColor);
      } 
    } 
