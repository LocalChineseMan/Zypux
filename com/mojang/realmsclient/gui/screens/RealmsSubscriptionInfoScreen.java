...
public class RealmsSubscriptionInfoScreen extends RealmsScreen {
  ...
  public void render(int xm, int ym, float a) {
    ...
    drawString(getLocalizedString("mco.configure.world.subscription.extendHere"), center, RealmsConstants.row(6), 10526880);
    int height = RealmsConstants.row(7);
    int textWidth = fontWidth("https://account.mojang.com/buy/realms");
    ...
    if (x1 <= xm && xm <= x2 && y1 <= ym && ym <= y2) {
      this.onLink = true;
      drawString("https://account.mojang.com/buy/realms", width() / 2 - textWidth / 2, height, 7107012);
    } else {
      this.onLink = false;
      drawString("https://account.mojang.com/buy/realms", width() / 2 - textWidth / 2, height, 3368635);
    } 
