...
public class PropsVectors {
  ...
  public void compact(CompactHandler compactor) {
    ...
    for (int j = 0; j < this.rows; j++) {
      int start = this.v[indexArray[j].intValue()];
      if (count < 0 || !areElementsSame(indexArray[j].intValue() + 2, this.v, indexArray[j - 1].intValue() + 2, valueColumns))
        count += valueColumns; 
      if (start == 1114112) {
    ...
    for (int k = 0; k < this.rows; k++) {
      int start = this.v[indexArray[k].intValue()];
      int limit = this.v[indexArray[k].intValue() + 1];
      if (count < 0 || !areElementsSame(indexArray[k].intValue() + 2, temp, count, valueColumns)) {
        count += valueColumns;
        System.arraycopy(this.v, indexArray[k].intValue() + 2, temp, count, valueColumns);
      } 
      if (start < 1114112)
  ...
  public IntTrie compactToTrieWithRowIndexes() {
    ...
    compact(compactor);
    return compactor.builder.serialize(new DefaultGetFoldedValue(compactor.builder), (Trie.DataManipulate)new DefaultGetFoldingOffset(null));
  }
  
  private static class PropsVectors {}
  ...
  public static interface CompactHandler {
    void setRowIndexForRange(int param1Int1, int param1Int2, int param1Int3);
    
    void setRowIndexForInitialValue(int param1Int);
    
    void setRowIndexForErrorValue(int param1Int);
    
    void startRealValues(int param1Int);
  }
}
