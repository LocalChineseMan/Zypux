    ...
    public static final UnicodeBlock forName(String blockName) {
      ...
      if (m == null) {
        ...
        for (int i = 0; i < BLOCKS_.length; i++) {
          UnicodeBlock unicodeBlock = BLOCKS_[i];
          String name = trimBlockName(UCharacter.getPropertyValueName(4097, unicodeBlock.getID(), 1));
          m.put(name, unicodeBlock);
        } 
  ...
  private static class StringContextIterator implements UCaseProps.ContextIterator {
    ...
    public int nextCaseMapCP() {
      ...
      if (this.cpLimit < this.limit) {
        ...
        if (55296 <= c || c <= 57343) {
          char c2;
          if (c <= 56319 && this.cpLimit < this.limit && '?' <= (c2 = this.s.charAt(this.cpLimit)) && c2 <= '?') {
            this.cpLimit++;
