      ...
      if (this.m_returnDelimiters_) {
        ...
        if (contains) {
          if (this.m_coalesceDelimiters_) {
            i = getNextNonDelimiter(this.m_nextOffset_);
          } else {
            i = this.m_nextOffset_ + UTF16.getCharCount(c);
            if (i == this.m_length_)
              i = -1; 
          } 
        } else {
          i = getNextDelimiter(this.m_nextOffset_);
        } 
        if (i < 0) {
          str1 = this.m_source_.substring(this.m_nextOffset_);
        } else {
          str1 = this.m_source_.substring(this.m_nextOffset_, i);
        } 
        this.m_nextOffset_ = i;
        return str1;
      } 
      int tokenlimit = getNextDelimiter(this.m_nextOffset_);
      if (tokenlimit < 0) {
        str = this.m_source_.substring(this.m_nextOffset_);
        this.m_nextOffset_ = tokenlimit;
      } else {
        str = this.m_source_.substring(this.m_nextOffset_, tokenlimit);
