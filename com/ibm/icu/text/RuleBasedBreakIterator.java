...
public class RuleBasedBreakIterator extends BreakIterator {
  ...
  private void makeRuleStatusValid() {
    if (!this.fLastStatusIndexValid) {
      ...
      Assert.assrt((this.fLastStatusIndexValid == true));
  ...
  private LanguageBreakEngine getEngineFor(int c) {
    ...
    try {
      switch (script) {
        ...
        case 22:
          if (getBreakType() == 1) {
            CjkBreakEngine cjkBreakEngine = new CjkBreakEngine(false);
            break;
          } 
        ...
        case 18:
          if (getBreakType() == 1) {
            CjkBreakEngine cjkBreakEngine = new CjkBreakEngine(true);
            break;
          } 
          this.fUnhandledBreakEngine.handleChar(c, getBreakType());
          eng = this.fUnhandledBreakEngine;
  ...
  private int handleNext() {
    if (this.fCachedBreakPositions == null || this.fPositionInCache == this.fCachedBreakPositions.length - 1) {
      ...
      if (this.fDictionaryCharCount > 1 && result - startPos > 1) {
        ...
        LanguageBreakEngine e = getEngineFor(CharacterIteration.current32(this.fText));
        if (e != null) {
          Stack<Integer> breaks = new Stack<Integer>();
          e.findBreaks(this.fText, startPos, result, false, getBreakType(), breaks);
