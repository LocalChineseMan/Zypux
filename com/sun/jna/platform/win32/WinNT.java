  ...
  public static class ACL extends Structure {
    ...
    public ACL(Pointer pointer) {
      ...
      for (int i = 0; i < this.AceCount; i++) {
        WinNT.ACCESS_ALLOWED_ACE aCCESS_ALLOWED_ACE;
        WinNT.ACCESS_DENIED_ACE aCCESS_DENIED_ACE;
        Pointer share = pointer.share(offset);
        byte aceType = share.getByte(0L);
        WinNT.ACCESS_ACEStructure ace = null;
        switch (aceType) {
          case 0:
            aCCESS_ALLOWED_ACE = new WinNT.ACCESS_ALLOWED_ACE(share);
            break;
          case 1:
            aCCESS_DENIED_ACE = new WinNT.ACCESS_DENIED_ACE(share);
            break;
          default:
            throw new IllegalArgumentException("Unknwon ACE type " + aceType);
        } 
        this.ACEs[i] = aCCESS_DENIED_ACE;
        offset += aCCESS_DENIED_ACE.AceSize;
      } 
    }
