...
public final class SelfSignedCertificate {
  ...
  public SelfSignedCertificate(String fqdn, SecureRandom random, int bits) throws CertificateException {
    ...
    try {
      arrayOfString = OpenJdkSelfSignedCertGenerator.generate(fqdn, keypair, random);
    } catch (Throwable t) {
      logger.debug("Failed to generate a self-signed X.509 certificate using sun.security.x509:", t);
      try {
        arrayOfString = BouncyCastleSelfSignedCertGenerator.generate(fqdn, keypair, random);
      } catch (Throwable t2) {
    ...
    this.certificate = new File(arrayOfString[0]);
    this.privateKey = new File(arrayOfString[1]);
  }
