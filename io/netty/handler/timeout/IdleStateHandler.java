...
public class IdleStateHandler extends ChannelDuplexHandler {
  ...
  private void initialize(ChannelHandlerContext ctx) {
    ...
    if (this.readerIdleTimeNanos > 0L)
      this.readerIdleTimeout = (ScheduledFuture<?>)loop.schedule(new ReaderIdleTimeoutTask(ctx), this.readerIdleTimeNanos, TimeUnit.NANOSECONDS); 
    if (this.writerIdleTimeNanos > 0L)
      this.writerIdleTimeout = (ScheduledFuture<?>)loop.schedule(new WriterIdleTimeoutTask(ctx), this.writerIdleTimeNanos, TimeUnit.NANOSECONDS); 
    if (this.allIdleTimeNanos > 0L)
      this.allIdleTimeout = (ScheduledFuture<?>)loop.schedule(new AllIdleTimeoutTask(ctx), this.allIdleTimeNanos, TimeUnit.NANOSECONDS); 
  }
  ...
  private final class ReaderIdleTimeoutTask implements Runnable {
    ...
    public void run() {
      ...
      if (nextDelay <= 0L) {
        IdleStateHandler.this.readerIdleTimeout = (ScheduledFuture<?>)this.ctx.executor().schedule(this, IdleStateHandler.this.readerIdleTimeNanos, TimeUnit.NANOSECONDS);
        try {
        ...
        IdleStateHandler.this.readerIdleTimeout = (ScheduledFuture<?>)this.ctx.executor().schedule(this, nextDelay, TimeUnit.NANOSECONDS);
      } 
    }
  }
  ...
  private final class WriterIdleTimeoutTask implements Runnable {
    ...
    public void run() {
      ...
      if (nextDelay <= 0L) {
        IdleStateHandler.this.writerIdleTimeout = (ScheduledFuture<?>)this.ctx.executor().schedule(this, IdleStateHandler.this.writerIdleTimeNanos, TimeUnit.NANOSECONDS);
        try {
          IdleStateEvent event;
        ...
        IdleStateHandler.this.writerIdleTimeout = (ScheduledFuture<?>)this.ctx.executor().schedule(this, nextDelay, TimeUnit.NANOSECONDS);
      } 
    }
  }
  ...
  private final class AllIdleTimeoutTask implements Runnable {
    ...
    public void run() {
      ...
      if (nextDelay <= 0L) {
        IdleStateHandler.this.allIdleTimeout = (ScheduledFuture<?>)this.ctx.executor().schedule(this, IdleStateHandler.this.allIdleTimeNanos, TimeUnit.NANOSECONDS);
        try {
          IdleStateEvent event;
        ...
        IdleStateHandler.this.allIdleTimeout = (ScheduledFuture<?>)this.ctx.executor().schedule(this, nextDelay, TimeUnit.NANOSECONDS);
      } 
    }
  }
}
