...
public final class PlatformDependent {
  ...
  public static <K, V> ConcurrentMap<K, V> newConcurrentHashMap() {
    if (CAN_USE_CHM_V8)
      return (ConcurrentMap<K, V>)new ConcurrentHashMapV8(); 
    return new ConcurrentHashMap<K, V>();
  }
  
  public static <K, V> ConcurrentMap<K, V> newConcurrentHashMap(int initialCapacity) {
    if (CAN_USE_CHM_V8)
      return (ConcurrentMap<K, V>)new ConcurrentHashMapV8(initialCapacity); 
    return new ConcurrentHashMap<K, V>(initialCapacity);
  }
  
  public static <K, V> ConcurrentMap<K, V> newConcurrentHashMap(int initialCapacity, float loadFactor) {
    if (CAN_USE_CHM_V8)
      return (ConcurrentMap<K, V>)new ConcurrentHashMapV8(initialCapacity, loadFactor); 
  ...
  public static <K, V> ConcurrentMap<K, V> newConcurrentHashMap(int initialCapacity, float loadFactor, int concurrencyLevel) {
    if (CAN_USE_CHM_V8)
      return (ConcurrentMap<K, V>)new ConcurrentHashMapV8(initialCapacity, loadFactor, concurrencyLevel); 
    return new ConcurrentHashMap<K, V>(initialCapacity, loadFactor, concurrencyLevel);
  }
  
  public static <K, V> ConcurrentMap<K, V> newConcurrentHashMap(Map<? extends K, ? extends V> map) {
    if (CAN_USE_CHM_V8)
      return (ConcurrentMap<K, V>)new ConcurrentHashMapV8(map); 
  ...
  public static <T> Queue<T> newMpscQueue() {
    return (Queue<T>)new MpscLinkedQueue();
  }
  ...
  private static int javaVersion0() {
    ...
    if (logger.isDebugEnabled())
      logger.debug("Java version: {}", Integer.valueOf(b)); 
    return b;
  }
  
  private static boolean hasUnsafe0() {
    boolean tryUnsafe, noUnsafe = SystemPropertyUtil.getBoolean("io.netty.noUnsafe", false);
