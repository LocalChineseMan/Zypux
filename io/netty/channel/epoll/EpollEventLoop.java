...
final class EpollEventLoop extends SingleThreadEventLoop {
  ...
  private final IntObjectMap<AbstractEpollChannel> ids = (IntObjectMap<AbstractEpollChannel>)new IntObjectHashMap();
  ...
  private void closeAll() {
    ...
    Collection<AbstractEpollChannel> channels = new ArrayList<AbstractEpollChannel>(this.ids.size());
    for (IntObjectMap.Entry<AbstractEpollChannel> entry : (Iterable<IntObjectMap.Entry<AbstractEpollChannel>>)this.ids.entries())
  ...
  private void processReady(long[] events, int ready) {
    for (int i = 0; i < ready; i++) {
      ...
      if (id == 0) {
        ...
        boolean read = ((ev & 0x1L) != 0L);
        boolean write = ((ev & 0x2L) != 0L);
        boolean close = ((ev & 0x8L) != 0L);
